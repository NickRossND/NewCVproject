...
for j=1:7
    ...
    Cnew = flip(Cnew,1);
    Cnew2 = flip(Cnew2,1);
%     saveas(left,savepath+names(2,j)+"OLD.png")
%     saveas(right,savepath+names(1,j)+"OLD.png")
...
% for j=1:7
%     i=10;
%     minimum_left = min(c3(:,:,j), [], 'all');
%     minimum_right = min(c32(:,:,j), [], 'all');
%     maximum_left = max(c3(:,:,j), [], 'all');
%     maximum_right = max(c32(:,:,j), [], 'all');
%     
%     if minimum_left < minimum_right
%         abs_min = minimum_left;
%     else
...
%     
%     figure(j+21);
%     right = imagesc(x32(i,:),y32(:,i),c32(:,:,j));
%     axis xy;
%     colormap jet;
%     colorbar;
%     caxis([abs_min, abs_max]);
%     saveas(left,savepath+'interp'+names(2,j)+".png")
%     saveas(right,savepath+'interp'+names(1,j)+".png")
% end
